defmodule Sweetroll2.Post.Generative do
  ...
  Recursively expands a list of URLs to include sub-posts generated by generative posts.
      ...
      iex> Generative.list_generated_urls(["/notes", "/tag", "/notes/dank-meme-7"], Map.merge(
      ...>  %{
      ...>    "/tag" => %Sweetroll2.Post{
  ...
  Looks up a post in posts, even if it's a sub-post generated by (a chain of) generative posts.
      ...
      iex> post = Generative.lookup("/tag/memes/page1", Map.merge(
      ...>  %{
      ...>    "/tag" => %Sweetroll2.Post{
      ...>      url: "/tag", type: "x-dynamic-tag-feed",
      ...>      props: %{ "filter" => [ %{"category" => ["{tag}"]} ] }
      ...
      iex> post = Generative.lookup("/tag",
      ...>  %{
      ...>    "/tag" => %Sweetroll2.Post{
      ...>      url: "/tag", type: "x-dynamic-tag-feed",
      ...>      props: %{ "filter" => [ %{"category" => ["{tag}"]} ] }
  ...
  (First argument is split parts of the URL, second is for recursion.)
      ...
      iex> Generative.find_first_matching_prefix([""], [], %{"/" => 1})
      {1, ""}

      iex> Generative.find_first_matching_prefix(["page1"], [], %{"/" => 1})
      {1, "/page1"}

      iex> Generative.find_first_matching_prefix(["one"], [], %{"/one" => 1})
      {1, ""}

      iex> Generative.find_first_matching_prefix(["one", "page2"], [], %{"/one" => 1})
      {1, "/page2"}

      iex> Generative.find_first_matching_prefix(["tag", "memes", "page69"], [], %{"/tag" => :tagpage})
      {:tagpage, "/memes/page69"}

      iex> Generative.find_first_matching_prefix(["memes", "page69"], [], %{"/tag" => :tagpage})
      nil
  """
